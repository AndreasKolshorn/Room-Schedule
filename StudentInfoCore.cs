...
/// <summary>
/// Summary description for StudentInfoCore
/// </summary>
public class StudentInfoCore
{
    ...
    const int CertType_CPR=1;
    const int CertType_FirstAid=2;
    const int CertType_WSP=3;
    const int CertType_TB=4;
    const int CertType_HepB=5;
    ...
    public StudentInfoCore()
	{
	}

    public StudentInfoCore(int studentUID)
	{
    ...
    private bool SQL_GetStudentProgramReqData(List<StudentInfoSQLBuffer> studentInfoSQLBuffer, int studentUID)
    {
        int bufferRow = 0;
        ...
        try
        {
            ...
            command.CommandText = "CSSP_GetStudentInfo";
            ...
            do
            {
                studentInfoSQLBuffer.Add(new
                    StudentInfoSQLBuffer(
                    bufferRow,
                    Convert.ToInt32(reader["StudentInfoID"]),
                    Convert.ToInt32(reader["StudentUID"]),

                    Convert.ToInt32(reader["StartProgram_TermCalendarID"]),
                    Convert.ToInt32(reader["ExpectedGrad_TermCalendarID"]),
                    Convert.ToBoolean(reader["ClinicContact"]),
                    Convert.ToBoolean(reader["Active"]),
                    reader["StudentInfo_NoteText"] as string,
                    Convert.ToInt32(reader["StudentCertTypeID"]),

                    reader["StudentCertTypeName"] as string,
                    Convert.ToDateTime(reader["AniversaryDate"]),
                    Convert.ToDateTime(reader["ExpireDate"]),

                    Convert.ToBoolean(reader["Waived"]),
        ...
        {
            errorCode = -1;
            errorMessage += "Request failed: StudentInfoCore.cs/CSSP_GetStudentInfo/Catch --> " + ex.Message;
            return false;
        }
    ...
    private bool SQL_GetStudentStatusData(List<StudentInfoStatus> studentInfoStatusSQLBuffer, List<StudentInfoStatus> studentInfoStatus, int studentUID)
    {
        string connStr = ConfigurationManager.ConnectionStrings["ClinicScheduleConnString"].ToString();
        SqlConnection connection = new System.Data.SqlClient.SqlConnection(connStr);
        System.Data.SqlClient.SqlCommand command = new System.Data.SqlClient.SqlCommand();
        ...
        try
        {
            ...
            // Build and execute command string
            command.CommandText = "CSSP_GetStudentStatus";
            command.CommandType = CommandType.StoredProcedure;
            command.Parameters.Add("@StudentUID", SqlDbType.Int).Value = studentUID;
            ...
            do
            {
                studentInfoStatusSQLBuffer.Add(new StudentInfoStatus(     
                    Convert.ToInt32(reader["StudentStatusID"]),
                    Convert.ToInt32(reader["StudentUID"]),
                    Convert.ToInt32(reader["ProgramsID"]),
                    Convert.ToInt32(reader["TermCalendarID"]),
                    Convert.ToInt32(reader["ProgramStatusID"]),
                    Convert.ToString(reader["TermCalendarName"]),
                    Convert.ToString(reader["ProgramStatusName"]),
                    Convert.ToString(reader["ProgramsCode"]),
                    Convert.ToString(reader["ProgramsDescription"])
                    ));

                studentInfoStatus.Add(new StudentInfoStatus(
                    Convert.ToInt32(reader["StudentStatusID"]),
                    Convert.ToInt32(reader["StudentUID"]),
                    Convert.ToInt32(reader["ProgramsID"]),
                    Convert.ToInt32(reader["TermCalendarID"]),
        ...
        {
            errorCode = -1;
            errorMessage += "Request failed: StudentInfoCore.cs/SQL_GetStudentStatusData/Catch --> " + ex.Message;
            return false;
        }
